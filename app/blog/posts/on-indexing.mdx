...
Specifically the kind that lets Google search 100 petabytes of web content before you finish typing. The kind that lets Shazam identify a song from a 5-second clip in noisy environment. The kind that every single AI startup is now desperately trying to build because their agents are hallucinating without it.

Let's get into it.

## The OG: How Google Indexes the Internet

Google obviously doesn't search the internet when you type a query. That would be impossibly slow so instead, it searches an index which is a massive, pre-computed data structure that serves as its representation of the entire web.
...
This is where we are today. Dozens of startups are aiming to be "the context layer for AI agents." They promise better retrieval, reduced hallucinations, and improved querying over existing agent tooling.

If you ask an agent "how do I fix the auth bug?", it can't answer unless it knows your auth code.

So we're back to indexing. But indexing code and internal docs is a different compared to the public web.
...
The systems that actually work (not just the demos) are using a multi-index approach:

1.  **Semantic Index (Vectors)**: For the vague, conceptual questions.
2.  **Keyword Index (BM25)**: For the precise, "I know what I'm looking for" queries.
3.  **Graph Index**: For the relationships. "Function A calls Function B which is defined in File C."

You query all three, merge the results, re-rank them, and *then* feed them to the LLM.

### More on BM25: A 40 Year Old Algorithm

**BM25** is a ranking function from the 80s that relies on keyword matching. It's being used now for exact precise matching.
...
For code, BM25 is often better than vectors. Developers use precise terminology. If I search for `UserAuthenticationService`, I want that exact class. BM25 gives me that. Vectors might give me `CustomerLoginResponse` because it's "semantically similar."

## The Hard Parts

### Incremental Reindexing

You can't reindex everything on every single change. Most architectures have background workers to watch for changes and update only affected documents.

However the classic swe hickup is dependency tracking. If file A imports file B, and B changes, do you reindex A? What about files that import A? How far does the cascade go?

We can build reference graphs to help so that when a 'symbol' changes, the graph tells you what depends on it. Reindex those dependencies, not the entire codebase.

But building and maintaining the graph is expensive. You need to parse code structure, track imports and especially watch out for circular dependencies.

### Version Tracking

You need to know not just "this file changed" but "this specific function signature changed from version A to version B."

That requires:
- Parsing code structure (AST analysis)
- Diffing at the symbol level not just the line level
- Storing version history in the index
- Filtering results by version when querying

At scale, this becomes a distributed systems problem. How often do you refetch? Every hour? Every day? How do you prioritize which sources to refresh first?
