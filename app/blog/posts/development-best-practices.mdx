...
- Direct dependency injection without over-engineering
- Simple factory pattern for dynamic agent creation
- Clean service layer without unnecessary abstraction
- Singleton database instances at module level (pragmatic choice)

### Error Handling & Resilience
- Structured exception hierarchy - custom exceptions for different error categories
- Result types over exceptions for expected failure cases (`Result[T, Error]` pattern)
...
- Normalized schemas with strategic denormalization (arrays) where PostgreSQL excels
- Row Level Security over application-level access control for data security
- Connection pooling managed by the database service, not the application
- Separate read/write models for different use cases (CQRS-lite pattern)
...
These practices prioritize developer productivity, system reliability, and maintainability while leveraging the strengths of modern development ecosystems. They're designed to scale from prototype to production and facilitate team collaboration.

The key is consistency - pick patterns that work for your team and apply them systematically across your codebase. These guidelines serve as a foundation that can be adapted based on specific project requirements and constraints.
